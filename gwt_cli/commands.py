...
def cmd_init_shell(shell_type=None):
    ...
    try:
        ...
        ref = importlib.resources.files("gwt_cli") / "gwt-shell.sh"
        content = ref.read_text(encoding="utf-8")
    except (AttributeError, FileNotFoundError):
        # Fallback for older Python versions
        try:
            content = importlib.resources.read_text("gwt_cli", "gwt-shell.sh")
        except Exception as e:
            print(f"Error loading gwt-shell.sh: {e}", file=sys.stderr)
            sys.exit(1)
