...
GWT_CLI_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "bin", "gwt-cli"))
...
class TestGitWorktreeTool(unittest.TestCase):
    def setUp(self):
        ...
        self.run_cmd(["git", "config", "user.email", "test@example.com"], cwd=self.remote_path)
    ...
    def test_new_worktree_success(self):
        ...
        self.assertEqual(os.path.realpath(res.stdout.strip()), os.path.realpath(expected_path))
        ...
        branch_name = self.run_cmd(["git", "branch", "--show-current"], cwd=expected_path).stdout.strip()
